...
const ClientAdminToggle: React.FC = () => {
    ...
    const [toggleAdmin, setToggleAdmin] = useState<boolean>(pathname.includes('admin') ? true : false);
    const [toggleClient, setToggleClient] = useState<boolean>(pathname.includes('client') ? true : false);
    ...
    function handleToggle(){
            ...
            let lastIndex = pathname.split("/").length
            let after = lastIndex <= 3 ? pathname.split('/').slice(2, lastIndex).join("/"): [];
            let portalView = pathname.includes("/admin") ? "client" : "admin";
            let newPathname = `/${portalView}/${after}`
            router.push(newPathname)
            
        }

    return(
        <div className="flex flex-col mr-4 w-44 h-16 pt-4 items-center z-50">
            {/* <span className="flex justify-center align-center p-2">View</span> */}
            {toggleAdmin &&
            <div className="inline-flex items-center mb-0 cursor-pointer" onClick={e => handleToggle()}>
                <span className="relative top-[1px] start-[2px] me-3 ms-3 text-sm font-medium text-transparent">Client</span>  
            ...
            {toggleClient &&
               <div className="inline-flex  items-center mb-0 cursor-pointer" onClick={e => handleToggle()} >
                ...
                <span className="relative top-[1px] start-[2px] me-3 ms-3 text-sm font-medium text-gray-900 dark:text-gray-300">Client</span> 
